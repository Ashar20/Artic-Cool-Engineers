...
function Parallx() {
  ...
  self.wrapperCenter = {
    x: ko.computed(function () {
      return self.wrapperOffset.left + $("#parallxWrapper").width() / 2;
    }, this),
  ...
  self.relativeMouse = {
    x: ko.computed(function () {
      return (
        (self.mouseX() - self.wrapperCenter.x()) * self.sensitivityMultiplier()
      );
    }, this),
    y: ko.computed(function () {
      return (
        (self.mouseY() - self.wrapperCenter.y()) *
        -1 *
        self.sensitivityMultiplier()
      );
    }, this),
  };
  self.origin = {
